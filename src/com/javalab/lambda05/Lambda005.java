 ...
 */
public class Lambda005 { // class s

	public static void main(String[] args) { // main s

		/*
		 * [1] 스레드 클래스를 바로 정의하여 사용함(익명 구현 객체 사용)
		 * 스레드의 생성자 Runnable 인터페이스를 구현한 익명 구현 객체를 바로 정의해서 객체 생성
		 * 선언하면서 바로 객체화 됨.
		 */
		...
		// [3] [2] 번을 람다식으로 표현
		// Runnable 생성자에 Runnable 인터페이스 타입 변수
		Runnable runnable2 = () ->{
			for ( int i = 1; i <= 10; i++) {
		 ...
		 * - 람다식을 통하여 생성된 익명 구현 객체가 스레드의 생성자의 매개변수로 전달
		 * - 람다식이 곧 Runnable 구현하는 익명 객체를 대신함.
		 */
		
		// new Thread(Runnable runnable1 = () -> { 이게 생략됨
